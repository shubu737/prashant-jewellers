...
const marqueeItems = [
  ...
  "Custom Crafting Available",
  "Since 1957",
  "Premium Gold & Diamonds",
...
export default function HomePage() {
  ...
  return (
    <div className="overflow-hidden bg-white">
      ...
      {/* ── Marquee ticker ── */}
      <div className="bg-[#111] text-white py-2.5 overflow-hidden">
        <div className="flex animate-marquee whitespace-nowrap gap-0">
          {[...marqueeItems, ...marqueeItems].map((item, i) => (
            <span key={i} className="mx-8 text-xs uppercase tracking-[0.3em] text-gold/80">
      ...
      {/* ── Hero (parallax) ── */}
      <section ref={heroRef} className="relative min-h-[92vh] flex items-center bg-[#0a0a0a] overflow-hidden">
        {/* bg image parallax */}
        ...
        {/* floating gold orbs */}
        <div className="absolute top-20 right-[20%] w-72 h-72 rounded-full bg-gold/10 blur-3xl animate-float pointer-events-none" />
        <div className="absolute bottom-20 right-[10%] w-48 h-48 rounded-full bg-gold/8 blur-2xl animate-float pointer-events-none" style={{ animationDelay: "2s" }} />

        <motion.div style={{ opacity: heroOpacity }} className="relative z-10 max-w-7xl mx-auto px-6 sm:px-8 w-full py-28">
          <div className="max-w-3xl">
            ...
            >
              ...
              <span className="text-gold text-xs uppercase tracking-[0.4em]">Since 1957 · Rawatbhata</span>
              <span className="gold-line" />
            </motion.div>
            ...
            <motion.h1
              ...
              className="text-6xl sm:text-7xl lg:text-8xl font-serif text-white leading-[1.05] tracking-tight"
            >
              Where Gold<br />
            ...
            <motion.p
              ...
              className="mt-6 text-lg text-white/70 max-w-xl leading-8"
            >
            ...
            >
              <Link
                href="/shop"
                className="group relative inline-flex items-center gap-2 rounded-full bg-gold px-8 py-4 text-sm font-semibold text-white overflow-hidden transition-all duration-300 hover:shadow-[0_0_30px_rgba(196,150,80,0.5)]"
              >
              ...
              <Link
                href="/contact"
                className="inline-flex items-center gap-2 rounded-full border border-white/30 bg-white/10 px-8 py-4 text-sm font-semibold text-white backdrop-blur-sm hover:bg-white/20 transition-all duration-300"
              >
                Book Consultation
              </Link>
            </motion.div>
            ...
            >
              {["BIS Hallmarked", "Lifetime Exchange", "Custom Crafting"].map((badge) => (
                <div key={badge} className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-white/60 uppercase tracking-widest">{badge}</span>
                </div>
              ))}
            </motion.div>
          </div>
      ...
      {/* ── Stats strip ── */}
      <Section className="bg-[#0d0d0d] py-14 border-y border-gold/10">
        <div className="max-w-7xl mx-auto px-6 sm:px-8">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
            {stats.map((stat, i) => (
              ...
              >
                <p className="text-4xl font-serif text-gradient-gold font-semibold">
                  <AnimatedCounter target={stat.value} suffix={stat.suffix} />
                </p>
                <p className="mt-2 text-xs uppercase tracking-[0.3em] text-white/40">{stat.label}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between mb-14">
            <div>
              ...
              <p className="text-xs uppercase tracking-[0.4em] text-gold mb-3">Timeless Artistry</p>
              <h2 className="text-4xl sm:text-5xl font-serif text-black">Signature Collections</h2>
            </div>
            <Link href="/shop" className="group inline-flex items-center gap-2 text-sm font-semibold text-black border-b border-black/20 pb-1 hover:border-gold hover:text-gold transition-all">
              Browse all collections <span className="group-hover:translate-x-1 transition-transform">→</span>
            </Link>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {signatureCollections.map((col, i) => (
              <motion.div
                ...
                className="group"
              >
                <Link href={col.cta} className="block overflow-hidden rounded-3xl luxury-card shadow-soft hover:shadow-premium transition-all duration-500">
                  <div className="relative h-72 overflow-hidden">
                    ...
                    <span className="absolute left-5 top-5 rounded-full border border-white/30 bg-black/40 backdrop-blur-sm px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.25em] text-white">
                      0{i + 1}
                    </span>
                  ...
                  <div className="px-6 py-6 space-y-3">
                    <p className="text-sm leading-7 text-gray-600">{col.description}</p>
      ...
      <Section className="py-24 bg-[#0d0d0d] relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-8 relative z-10">
          <div className="text-center mb-16">
            ...
            <p className="text-xs uppercase tracking-[0.4em] text-gold mb-3">Why Choose Us</p>
            <h2 className="text-4xl sm:text-5xl font-serif text-white">The Prashant Promise</h2>
          </div>
          <div className="grid gap-6 md:grid-cols-3">
            {benefitFeatures.map((f, i) => (
              <motion.div
                ...
                className="rounded-3xl border border-gold/15 bg-white/5 p-8 text-center backdrop-blur-sm hover:border-gold/40 hover:bg-white/8 transition-all duration-400"
              >
                <div className="text-3xl text-gold mb-4 animate-gold-pulse">{f.icon}</div>
                <h3 className="text-xl font-serif text-white mb-3">{f.title}</h3>
                <p className="text-sm text-white/50 leading-7">{f.subtitle}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-8">
          <div className="grid gap-16 lg:grid-cols-2 items-center">
            <motion.div variants={fadeLeft} transition={{ duration: 0.8 }} className="relative">
              <div className="relative overflow-hidden rounded-[2.5rem] shadow-premium aspect-[4/5]">
              ...
              <motion.div
                ...
                className="absolute -bottom-6 -right-6 rounded-2xl bg-gold p-6 shadow-premium text-center"
              >
                <p className="text-3xl font-serif font-bold text-white">60+</p>
                <p className="text-xs uppercase tracking-widest text-white/80 mt-1">Years of Trust</p>
              </motion.div>
            </motion.div>

            <motion.div variants={fadeRight} transition={{ duration: 0.8 }} className="space-y-8">
              <span className="gold-line block" />
              <p className="text-xs uppercase tracking-[0.4em] text-gold">Since 1957</p>
              <h2 className="text-4xl sm:text-5xl font-serif text-black leading-tight">
                The Art of<br />Prashant Jewellers
              </h2>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={item.label} className="rounded-2xl border border-gold/15 bg-white p-6 shadow-soft hover:border-gold/40 transition-colors">
                    <p className="text-xs uppercase tracking-[0.3em] text-gold mb-2">{item.label}</p>
      ...
      <Section className="py-24 bg-[#111] relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-8 relative z-10">
          <div className="grid gap-16 lg:grid-cols-2 items-center">
            <motion.div variants={fadeLeft} transition={{ duration: 0.8 }} className="space-y-8 order-2 lg:order-1">
              <span className="gold-line block" />
              <p className="text-xs uppercase tracking-[0.4em] text-gold">Founder Story</p>
              <h2 className="text-4xl sm:text-5xl font-serif text-white leading-tight">
                A Legacy Shaped<br />by Generations
              </h2>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={item.label} className="rounded-2xl border border-gold/20 bg-white/5 p-6">
                    <p className="text-xs uppercase tracking-[0.35em] text-gold/70 mb-2">{item.label}</p>
            ...
            <motion.div variants={fadeRight} transition={{ duration: 0.8 }} className="relative order-1 lg:order-2">
              <div className="overflow-hidden rounded-[2.5rem] shadow-premium aspect-[4/5]">
                <img src="/images/founder/krish-soni.jpg" alt="Founder Mr. Krish Soni" className="h-full w-full object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent" />
              </div>
              <div className="absolute -top-4 -left-4 w-24 h-24 rounded-full border border-gold/30 animate-float" />
              <div className="absolute -bottom-4 -right-4 w-16 h-16 rounded-full border border-gold/20 animate-float" style={{ animationDelay: "1.5s" }} />
            </motion.div>
          </div>
        </div>
      </Section>
      ...
      <Section className="py-24 bg-[#faf6f0]">
        <div className="max-w-7xl mx-auto px-6 sm:px-8">
          <div className="text-center mb-16">
            <span className="gold-line mx-auto mb-4 block" />
            <p className="text-xs uppercase tracking-[0.4em] text-gold mb-3">Client Love</p>
            <h2 className="text-4xl sm:text-5xl font-serif text-black">What Our Clients Say</h2>
          </div>
      ...
      <Section className="relative py-32 overflow-hidden bg-[#0a0a0a]">
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-6 sm:px-8 text-center">
          <motion.div variants={fadeUp} transition={{ duration: 0.8 }}>
            <span className="gold-line mx-auto mb-6 block" />
            <p className="text-xs uppercase tracking-[0.4em] text-gold mb-4">Browse More</p>
            <h2 className="text-5xl sm:text-6xl font-serif text-white leading-tight mb-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                href="/shop"
                className="group relative inline-flex items-center justify-center gap-2 rounded-full bg-gold px-10 py-4 text-sm font-semibold text-white overflow-hidden hover:shadow-[0_0_40px_rgba(196,150,80,0.5)] transition-all duration-300"
              >
              ...
              <a
                ...
                className="inline-flex items-center justify-center gap-2 rounded-full border border-white/20 bg-white/10 px-10 py-4 text-sm font-semibold text-white backdrop-blur-sm hover:bg-white/20 transition-all duration-300"
              >
                WhatsApp Us
              </a>
            </div>
